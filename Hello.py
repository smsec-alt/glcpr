...
def main():   
    with st.sidebar:
        ...
        df = gcs.read_csv(f'global_cash/cash_prices_{country_name.lower()}.csv', parse_dates=['TRADEDATE'])
        # df = pd.read_csv(f'./data/cash_prices_{country_name.lower()}.csv', parse_dates=['TRADEDATE'])
    ...
    variable_name = 'Cash Prices' if add_country != 'USA' else add_secondary_variable
    ...
    if (len(all_categories) > 1) and (add_2legs):
        ...
        st.plotly_chart(get_chart(df_all, 'TRADEDATE', 'Result', f'{country_name} -- {leg1}-{leg2} {variable_name} {add_operation}',
                                  labels={'Result':add_operation, 'TRADEDATE':''}))

        st.plotly_chart(get_seasonality_chart(df_all, 'TRADEDATE', 'Result', f'{country_name} -- {leg1}-{leg2} {variable_name} {add_operation} Seasonality',
                                              labels={'Result':add_operation, 'DATE':'', 'YEAR':'Year'}))
        
    else:    
        subdf = df.query('NAME==@add_category & TRADEDATE>=@start & TRADEDATE<=@end')
        st.plotly_chart(get_chart(subdf, 'TRADEDATE', 'CLOSE', f'{country_name} -- {add_category} {variable_name}', logs=add_logs,
                                  labels={'CLOSE':add_category, 'TRADEDATE':'', 'YEAR':'Year'}))
        st.plotly_chart(get_seasonality_chart(subdf, 'TRADEDATE', 'CLOSE', f'{country_name} -- {add_category} {variable_name} Seasonality',
                                              labels={'CLOSE':add_category, 'TRADEDATE':'', 'YEAR':'Year'}))
